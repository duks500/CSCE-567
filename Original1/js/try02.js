...
var redrawChart = function(targetID, newdata) {
    ...
    var chartRow = svg.selectAll("g.chartRow")
        .data(newdata, function(d){ return d.chain});
    var newRow = chartRow
        .enter()
        .append("g")
        .attr("class", "chartRow")
    ...
    chartRow.select(".label").transition()
        ...
        .tween("text", function(d) {
            var i = d3.interpolate(+this.textContent.replace(/\,/g,''), +d.sales);
            return function(t) {
    ...
    chartRow.transition()
        .delay(delay)
        .duration(900)
        .attr("transform", function(d){ return "translate(0," + y(d.chain) + ")"; });
};
...
var pullData = function(settings,callback){
    d3.json("data/data.json", function (err, data){
        ...
        data.forEach(function(d,i){
            var newSales = d.sales + Math.floor((Math.random()*10) - 5)
            newData[i].sales = newSales <= 0 ? 10 : newSales
        })
